...
class Ferro2DSim:
    ...
    def __init__(self, n=10, gamma=1.0,
        ...
        self.temp = temp
        self.T0 = 400
        self.stochastic = stochastic  # whether to use stochastic gradients
        self.stochastic_frac = 0.10   # fraction of sites updated per step (was 0.25)
        ...
        # Do not set seterr globally — underflow in exp is harmless (rounds to 0)
        # and raising on it breaks callers that have a more permissive error policy.
    ...
    def runSim(self, calc_pr=False, verbose=True):
        # Use the vectorized solver (all modes supported); fall back to the
        # original per-site solver only if something unexpected happens.
        try:
            dpdt, pnew = self.calcODE_fast(
                stochastic_frac=self.stochastic_frac if self.stochastic else 1.0,
                verbose=verbose,
            )
        except Exception:
    ...
    def _lk_energy(self, py, px, k_mat, dep_mat, Eloc_y, Eloc_x, Ey, Ex):
        """Compute total LK free energy for current polarisation state."""
        N = py.size
        sum_py = (np.roll(py, 1, 0) + np.roll(py, -1, 0) +
                  np.roll(py, 1, 1) + np.roll(py, -1, 1))
        sum_px = (np.roll(px, 1, 0) + np.roll(px, -1, 0) +
                  np.roll(px, 1, 1) + np.roll(px, -1, 1))
        total_py = float(py.sum())
        total_px = float(px.sum())
        loc_y = Ey - dep_mat * total_py / N + Eloc_y
        loc_x = Ex - dep_mat * total_px / N + Eloc_x
        if self.mode == 'uniaxial':
            F = (self.alpha1 / 2) * py ** 2 + (self.alpha2 / 4) * py ** 4
            F += 0.5 * k_mat * (py - sum_py / 4) ** 2
            F -= loc_y * py
        elif self.mode == 'squareelectric':
            F = ((self.alpha1 / 2) * (px**2 + py**2) +
                 (self.alpha2 / 4) * (px**4 + py**4))
            F += 0.5 * k_mat * ((py - sum_py/4)**2 + (px - sum_px/4)**2)
            F -= loc_y * py + loc_x * px
        else:  # tetragonal / rhombohedral
            F = (self.alpha1 * (px**2 + py**2) +
                 self.alpha2 * (px**4 + py**4) +
                 self.alpha3 * px**2 * py**2)
            F += 0.5 * k_mat * ((py - sum_py/4)**2 + (px - sum_px/4)**2)
            F -= loc_y * py + loc_x * px
        return float(F.sum())

    def calcODE_fast(self, stochastic_frac=0.10, verbose=False,
                     energy_tol=1e-4, check_every=10, max_steps=None):
        """Vectorised LK ODE solver with energy-convergence early stopping.

        Runs until either all ``time_vec`` steps are consumed or the relative
        change in LK free energy drops below ``energy_tol`` (checked every
        ``check_every`` steps).  Pass ``max_steps`` to cap the run independent
        of ``time_vec`` length.

        Speed vs original calcODE:
          - Eliminates the O(N) inner ``for i`` loop → vectorised NumPy per step
          - O(1) ``if i in selected_set`` replaced by boolean mask
          - stochastic_frac=0.10 (10 % of sites per step, was 25 %)
          - getPmat reads from _pnew_cache (array slice) not per-atom getP()
        """
        n = self.n
        N = n * n
        T = len(self.time_vec)
        ...
        k_mat   = np.array(self.k,        dtype=float).reshape(n, n)
        dep_mat = np.array(self.dep_alpha, dtype=float).reshape(n, n)
        Eloc_x  = np.array([e[0] for e in self.Eloc], dtype=float).reshape(n, n)
        Eloc_y  = np.array([e[1] for e in self.Eloc], dtype=float).reshape(n, n)

        init_p = np.array([self.atoms[i].getP(0) for i in range(N)], dtype=float)
        px = init_p[:, 0].reshape(n, n)
        py = init_p[:, 1].reshape(n, n)

        # Store only what we need: full pnew for getPmat compatibility
        pnew = np.zeros((N, 2, T), dtype=float)
        pnew[:, 0, 0] = px.ravel()
        pnew[:, 1, 0] = py.ravel()

        n_update = max(1, int(N * stochastic_frac))
        prev_energy = None
        last_t = 0

        for t in range(1, T):
            dt = float(self.time_vec[min(t, len(self.time_vec) - 1)] -
                       self.time_vec[min(t - 1, len(self.time_vec) - 1)])
            ...
            total_py = float(py.sum())
            total_px = float(px.sum())
            Ey = float(self.appliedE[min(t, len(self.appliedE) - 1), 1])
            Ex = float(self.appliedE[min(t, len(self.appliedE) - 1), 0])

            loc_y = Ey - dep_mat * total_py / N + Eloc_y
            loc_x = Ex - dep_mat * total_px / N + Eloc_x

            if self.mode == 'uniaxial':
                dpy = -self.gamma * (self.alpha2 * py**3 + self.alpha1 * py +
                                     k_mat * (py - sum_py / 4) - loc_y)
                dpx = np.zeros_like(px)
            elif self.mode == 'squareelectric':
                dpy = -self.gamma * (self.alpha2 * py**3 + self.alpha1 * py +
                                     k_mat * (py - sum_py / 4) - loc_y)
                dpx = -self.gamma * (self.alpha2 * px**3 + self.alpha1 * px +
                                     k_mat * (px - sum_px / 4) - loc_x)
            elif self.mode in ('tetragonal', 'rhombohedral'):
                dpy = -self.gamma * (2*self.alpha1*py + 4*self.alpha2*py**3 +
                                     2*self.alpha3*px**2*py +
                                     k_mat * (py - sum_py / 4) - loc_y)
                dpx = -self.gamma * (2*self.alpha1*px + 4*self.alpha2*px**3 +
                                     2*self.alpha3*py**2*px +
                                     k_mat * (px - sum_px / 4) - loc_x)
            else:
                raise ValueError(f"Unsupported mode: {self.mode}")

            if stochastic_frac < 1.0:
                mask = np.zeros(N, dtype=bool)
                mask[np.random.choice(N, n_update, replace=False)] = True
                mask = mask.reshape(n, n)
                py = np.where(mask, np.clip(py + dpy * dt, -10, 10), py)
                px = np.where(mask, np.clip(px + dpx * dt, -10, 10), px)
            else:
                py = np.clip(py + dpy * dt, -10, 10)
                px = np.clip(px + dpx * dt, -10, 10)

            pnew[:, 1, t] = py.ravel()
            pnew[:, 0, t] = px.ravel()
            last_t = t

            # Energy convergence check every check_every steps
            if energy_tol is not None and t % check_every == 0:
                energy = self._lk_energy(py, px, k_mat, dep_mat,
                                         Eloc_y, Eloc_x, Ey, Ex)
                if prev_energy is not None:
                    rel_change = abs(energy - prev_energy) / (abs(prev_energy) + 1e-30)
                    if rel_change < energy_tol:
                        if verbose:
                            print(f"  Converged at step {t}: rel_ΔE={rel_change:.2e}")
                        break
                prev_energy = energy

        # Trim pnew to the steps actually run
        pnew = pnew[:, :, :last_t + 1]
        self.time_steps = last_t + 1

        # Update atoms for final step only
        for i in range(N):
            self.atoms[i].setP(last_t, (float(pnew[i, 0, last_t]),
                                        float(pnew[i, 1, last_t])))

        self._pnew_cache = pnew
        return np.zeros_like(pnew), pnew
    ...
    def calcODE(self, stochastic = True, verbose=True, time_vec = None, start_p = None):
        ...
        for t in tqdm(np.arange(2, len(time_vec)), disable = disable_tqdm):
            ...
            if stochastic:
                # Use a set for O(1) membership test (was O(N) with a list)
                selected_set = set(
                    np.random.choice(N, max(1, int(N * self.stochastic_frac)),
                                     replace=False).tolist()
                )
            else:
                selected_set = set(range(N))
    ...
    def getPmat(self, time_step=None):
        """Return polarisation matrix of shape (2, t, N, N) or (2, N, N) at a given step.

        When calcODE_fast has been used, reads directly from the cached pnew array
        (an array slice) rather than iterating over atom objects — O(1) vs O(N·T).
        """
        n = self.n

        if hasattr(self, '_pnew_cache'):
            pnew = self._pnew_cache   # shape (N*N, 2, T)
            if time_step is None:
                T = pnew.shape[2]
                Pmat = np.zeros((2, T, n, n), dtype=float)
                Pmat[0] = pnew[:, 0, :].T.reshape(T, n, n)
                Pmat[1] = pnew[:, 1, :].T.reshape(T, n, n)
                self.Pmat = Pmat
            else:
                Pmat = np.zeros((2, n, n), dtype=float)
                Pmat[0] = pnew[:, 0, time_step].reshape(n, n)
                Pmat[1] = pnew[:, 1, time_step].reshape(n, n)
            return Pmat

        # Original slow path (used when calcODE was called instead of calcODE_fast)
        if time_step is None:
            Pmat = np.zeros(shape=(2, self.time_steps, n, n))
            for ind in range(self.time_steps):
                Pvals = np.zeros(shape=(2, n * n))
                for i in range(n * n):
                    Pvals[:, i] = self.atoms[i].getP(ind)
                Pmat[:, ind, :, :] = Pvals.reshape(2, n, n)
            self.Pmat = Pmat
        else:
            Pmat = np.zeros(shape=(2, n, n))
            Pvals = np.zeros(shape=(2, n * n))
            for i in range(n * n):
                Pvals[:, i] = self.atoms[i].getP(time_step)
            Pmat[:, :, :] = Pvals.reshape(2, n, n)
